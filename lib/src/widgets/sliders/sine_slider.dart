...
class _RenderSineSlider extends RenderBox implements MouseTrackerAnnotation {
  ...
  double _f({required double a, required double l, required double x}) {
  ...
  void _paintTrack(PaintingContext context, Rect rect) {
    ...
    for (double x = 0; x <= thumbCenter; x += 0.05) {
      if (x == 0) {
        leftPath.moveTo(x, _f(a: amplitude, l: l, x: x) * -1);
      } else {
        leftPath.lineTo(x, _f(a: amplitude, l: l, x: x) * -1);
      }
    }
    ...
    for (double x = thumbCenter; x <= w; x += 0.05) {
      if (x == thumbCenter) {
        rightPath.moveTo(x, _f(a: amplitude, l: l, x: x) * -1);
      } else {
        rightPath.lineTo(x, _f(a: amplitude, l: l, x: x) * -1);
      }
    }
    ...
    context.canvas.drawRRect(
      ...
      Paint()
        ..shader = ui.Gradient.linear(offset, offset.translate(w, 0), const [
          Color(0xFF38393B),
          Color(0xFF37383A),
        ])
        ..strokeCap = StrokeCap.round,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final trackRect = _getTrackRect(parentBox: this, offset: offset);
  ...
  void handleEvent(PointerEvent event, BoxHitTestEntry entry) {
    ...
    if (event is PointerHoverEvent) {
      ...
      final trackRect = _getTrackRect(parentBox: this, offset: Offset.zero);
  ...
  void _startInteraction(Offset globalPosition) {
    ...
    _onChangeStart?.call(_discretize(value));
    ...
    _controller.forward(from: 0).whenComplete(() {
      ...
      _onChangeEnd?.call(_discretize(_valueTween.end ?? 0));
    });
  }
  ...
  void _endInteraction() {
    ...
    _onChangeEnd?.call(_discretize(_currentDragValue));
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    _onChanged(_discretize(_currentDragValue));
  }
